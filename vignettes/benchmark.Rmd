...
for (n in 1:length(tissue_both)) {
  ...
  if (sum(is.na(s_f@meta.data$free_annotation)) + sum(is.na(s_d@meta.data$free_annotation)) < (nrow(s_f@meta.data) + nrow(s_d@meta.data)) * 0.25) {
    ...
    res <- clustify(s_d, avg, seurat_out = F, cluster_col = "free_annotation", query_genes = DE_genes$Gene, per_cell = T)
  } else {
    print("use cell_ontology_class")
    if (length(unique(s_f@meta.data$cell_ontology_class)) < length(unique(s_d@meta.data$cell_ontology_class))) {
      s_temp <- s_d
    ...
    avg <- use_seurat_comp(s_f, cluster_col = "cell_ontology_class")
    res <- clustify(s_d, avg, seurat_out = F, cluster_col = "cell_ontology_class", query_genes = DE_genes$Gene, per_cell = T)
  }  #avg <- cbind(avg, avg2)
  res2 <- cor_to_call(res) 
  res2$cluster <- ids
  res2 <- res2 %>% select(-r) %>% group_by_all() %>% tally() %>% mutate(call = ifelse(cluster == type, 1, 0))
