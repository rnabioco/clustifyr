...
for (n in 1:length(tissue_both)) {
  ...
  res2 <- res2 %>%
    group_by_all() %>%
    tally() %>%
    mutate(call = ifelse(cluster == type, 1, 0))
...
resall2 <- resall2 %>%
  mutate(finalcall = ifelse(call == 1, 1, ifelse(str_c(cluster, type) %in% str_c(resall2negcor_all$cluster, resall2negcor_all$type), 1, 0))) %>%
  replace_na(list(finalcall = 0))
resall2
...
resall2 <- resall %>%
  filter(!is.na(old)) %>%
  mutate(call = ifelse(old == type, 1, 0)) %>%
  mutate(finalcall = ifelse(call == 1, 1, ifelse(str_c(old, type) %in% str_c(resall2negcor_all$cluster, resall2negcor_all$type), 1, 0)))
...
clustify(scmap::yan, yan_ref, scmap::ann, cluster_col = "cell_type1", per_cell = TRUE) %>%
  cor_to_call() %>%
  left_join(scmap::ann %>% rownames_to_column("cluster")) -> res
