...
pbmc4k_matrix_full <- readRDS(here("full_pbmc4k_matrix"))
pbmc4k_meta_full <- readRDS(here("full_pbmc4k_meta"))
microarray_ref <- clustifyrdata::hema_microarray_matrix

start <- proc.time()
res <- clustify(pbmc4k_matrix_full, 
                microarray_ref, 
                pbmc4k_meta_full, 
...
```{r test_single, include = F, eval=F}
times <- list()
for (n in 1:length(tissue_both)) {
  print(tissue_both[n])
  s_d <- load_obj(paste0("/Users/rf/", files_facs_fil[n]))
  ...
  if (sum(is.na(s_f@meta.data$free_annotation)) + sum(is.na(s_d@meta.data$free_annotation)) < (nrow(s_f@meta.data) + nrow(s_d@meta.data)) * 0.25) {
    ...
    res2 <- cor_to_call(res, metadata = s_d@meta.data %>% rownames_to_column("id"), col = "id", collapse_to_cluster = "free_annotation") %>% distinct(type, type2)
  } else {
    print("use cell_ontology_class")
    if (length(unique(s_f@meta.data$cell_ontology_class)) < length(unique(s_d@meta.data$cell_ontology_class))) {
      s_temp <- s_d
    ...
    res2 <- cor_to_call(res, metadata = s_d@meta.data %>% rownames_to_column("id"), col = "id", collapse_to_cluster = "cell_ontology_class") %>% distinct(type, type2)
  }
...
```{r test_single_rm0, include = F, eval=F}
times <- list()
for (n in 1:length(tissue_both)) {
  print(tissue_both[n])
  s_d <- load_obj(paste0("/Users/rf/", files_facs_fil[n]))
...
```{r test_single_alra, include = F, eval=F}
...
```{r load2, message=FALSE, warning=FALSE, include = F, eval=F}
library(here)
library(clustifyR)
library(tidyverse)
...
```{r test_cluster, include = F, eval=F}
# ref <- readRDS("tabula_muris_ref_drop_042119.rds")
times <- list()
for (n in 1:length(tissue_both)) {
  print(tissue_both[n])
...
```{r test_cluster_singlerREF, include = F, eval=F}
# ref <- readRDS("tabula_muris_ref_drop_042119.rds")
...
```{r test_sctransform, include = F, eval=F, warning=F}
