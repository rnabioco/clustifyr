...
#Get overlapping genes matrix in the same order
shared.genes <- rownames(pbmc4k_avg)[rownames(pbmc4k_avg) %in% rownames(pbmc_bulk_matrix)]
shared.genes <- shared.genes[order(shared.genes)]
singlecell.clusters <- pbmc4k_avg[shared.genes,]
names(singlecell.clusters) <- c(
   "CD4 T cells, 1",
   "CD14+ Monocytes",
   "B cells",
   "CD4 T cells, 2",
