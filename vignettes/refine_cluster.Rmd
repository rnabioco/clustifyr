...
where $S(\vec{x\ }, \vec{B_i})$ represents the similarity score between $\vec{x\ }$ and $\vec{B_i}$. One example of similarity score is the correlation computed by `clustify()` function. For clusters without reference data, we assign a default similarity score. $\epsilon$ is a user-defined weighting parameter. 

The latter probability is modeled as a Gaussian distribution:
