...
Yes, simply adding columns to a reference matrix works to expand it. We also provide `build_atlas`, which can be run like `build_atlas(matrix_objs = list(reference1, reference2, reference3, ...), genes_fn = clustifyr::human_genes_10x)`.
