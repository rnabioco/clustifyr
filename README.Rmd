...
`clustify()` can also take a clustered `seurat` object (both v2 and v3) and assign identities.
...
New reference matrix can be made directly from `seurat` object as well. Other scRNAseq experiment object types are supported as well.
...
`clustify_lists()` handles identity assignment of matrix or `seurat` object based on marker gene lists.
...
Also see list for individual downloads at `vignette("download_refs", package = "clustifyr")`
