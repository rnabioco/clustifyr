...
10. **Can I extract the overlapping genes from`clustify_lists()`?**
This was an requested and added feature. `details_out = TRUE` will output 2 matrices, the normal score, and a matrix containing all genes overlapping in the corresponding ref vs query pairs.

```{r example_outputshared, warning=F, message=F}
res <- clustify_lists(
  pbmc_matrix_small,
  metadata = pbmc_meta,
  cluster_col = "classified",
...
11. **Why is the default setting `per_cell = FALSE`?**
...
12. **Can I use multiple references in the same clustify run?**
...
13. **Does clustifyr work for spatial scRNA-seq data?**
...
14. **Can I pull out additional information on what gene signatures don't match the reference clusters?**
