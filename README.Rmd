...
3. I have "CLASH" in many of my final calls, why is that?
"CLASH" indicates ties in the correlation values. In practice, this should be very rare unless the amount of query genes is very (dangerously) low (use `verbose = TRUE` in `clustify` for more information).

4. I need help troubleshooting unknown errors in my reference building/clustifying.
As we try to provide better error messaging, it is still important to note in general the most error-prone step is at designating the column in the metadata that contains clustering information. This is generally the `cluster_col` argument.
