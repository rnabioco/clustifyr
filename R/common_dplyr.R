...
cor_to_call <- function(cor_mat,
    ...
    if (nrow(df_temp) != nrow(correlation_matrix)) {
        ...
        df_temp2 <- df_temp
        df_temp_full <-
            dplyr::distinct_at(df_temp, vars(-!!dplyr::sym("type")), .keep_all = TRUE)
    } else {
...
call_to_metadata <- function(res,
    ...
    if (per_cell == FALSE) {
        ...
        if (!(all(unique(df_temp[[cluster_col]]) %in% unique(metadata[[cluster_col]])))) {
            stop(
                "cluster_col from clustify step and joining to metadata step are not the same",
                call. = FALSE
            )
        }
...
collapse_to_cluster <- function(res,
    ...
    df_temp_full2 <-
        dplyr::group_by(df_temp_full, !!dplyr::sym("type"), !!dplyr::sym("cluster"))
    df_temp_full2 <-
    ...
    df_temp_full2 <-
        dplyr::group_by(df_temp_full2, !!dplyr::sym("cluster"))
    df_temp_full2 <-
        dplyr::arrange(df_temp_full2, desc(n), desc(sum))
    df_temp_full2 <-
        dplyr::filter(df_temp_full2, !!dplyr::sym("type") != paste0("r<", threshold, ", unassigned"))
    df_temp_full2 <- dplyr::slice(df_temp_full2, 1)
    df_temp_full2 <-
        dplyr::rename(df_temp_full2, !!cluster_col := cluster)
...
cor_to_call_rank <- function(cor_mat,
    ...
    df_temp <-
        dplyr::mutate(dplyr::group_by_at(df_temp, 1), rank = dplyr::dense_rank(desc(!!dplyr::sym("r"))))
