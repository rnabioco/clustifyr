...
meta <- clustifyrdata::pbmc_meta %>% rownames_to_column("id") %>% mutate(classified = ifelse(str_detect(clustifyrdata::pbmc_meta$classified, "CD4"), "CD4 T", as.character(classified))) %>% column_to_rownames("id")
...
ggsave("Fig1A.pdf", cowplot::plot_grid(a,b,c,d, ncol = 1, labels = c("orig","microarray_ref","sorted_bulk_ref","scRNA-seq_ref")), width =7, height = 15, dpi = 400)
```
...
for (n in 1:length(tissue_both)) {
  ...
  if (sum(is.na(s_f@meta.data$free_annotation)) + sum(is.na(s_d@meta.data$free_annotation)) < (nrow(s_f@meta.data) + nrow(s_d@meta.data)) * 0.25) {
    ...
    res <- clustify(s_d, avg, seurat_out = F, cluster_col = "free_annotation", dr = "tsne")
  } else {
...
# resall2negcor_all <- rbind(resall2negcor_all,resall2negcor2) %>% distinct(type, cluster, .keep_all = T)
...
singler_s <- function(s, col, ref) {
  singler = CreateSinglerObject(s@data, annot = NULL, "whatever", min.genes = 0,
    ...
    fine.tune = F, do.signatures = F, clusters = s@meta.data[[col]], do.main.types = F, 
...
# resall2negcor_all <- bind_rows(list(resall2negcor_all[,1:5],resall2negcor2)) %>% distinct(type, cluster, .keep_all = T)
# resall2negcor_all <- saveRDS(resall2negcor_all, "/Users/rf/cluster/devel/3rd/clustifyR/resall2negcor_all")
...
for (n in 1:length(tissue_both)) {
  ...
  if (sum(is.na(s_f@meta.data$free_annotation)) + sum(is.na(s_d@meta.data$free_annotation)) < (nrow(s_f@meta.data) + nrow(s_d@meta.data)) * 0.25) {
    ...
    fits <- M3DropDropoutModels(Normalized_data$data, suppress.plot = T)
    ...
    avg <- seurat_ref(s_f, cluster_col = "free_annotation")
    res <- clustify(s_d, avg, seurat_out = F, cluster_col = "free_annotation", dr = "tsne", query_genes = markers_M3Drop$Gene)
  } else {
    print("use cell_ontology_class")
    if (length(unique(s_f@meta.data$cell_ontology_class)) < length(unique(s_d@meta.data$cell_ontology_class))) {
      s_temp <- s_d
    ...
    avg <- seurat_ref(s_f, cluster_col = "cell_ontology_class")
    res <- clustify(s_d, avg, seurat_out = F, cluster_col = "cell_ontology_class", dr = "tsne", query_genes = markers_M3Drop$Gene)
  }  #avg <- cbind(avg, avg2)
  res2 <- cor_to_call(res)
  end <- proc.time()
  times[n] = end - start
...
```{r}
f <- readRDS("/Users/rf/cluster/devel/4th/clu4th/vignettes/bench_clus.rds")
a <- ggplot(f, aes(x = method, y = per, fill = method)) + 
  geom_boxplot() +
  #geom_jitter() +
...
b <- ggplot(f, aes(x = method, y = as.numeric(time), fill = method)) + 
  geom_boxplot() +
  #geom_jitter() +
  cowplot::theme_cowplot() + 
  theme(axis.title.y.right = element_text(color = "red"), axis.text.x = element_text(size = 12, angle = 90, hjust = 1, vjust=0.5)) +
...
ggsave("Fig1B.pdf", cowplot::plot_grid(a,b, nrow = 1), width =7, height = 7, dpi = 400)
```
...
g1
ggsave("Fig2E.pdf", g1, width = 7, height = 7, dpi = 400)

r <- readRDS("res_pbmcbench.rds")
r2 <- r %>% separate(run, sep = "_", into = c("sample", "exp", "sub"))
...
g2
#ggsave("Fig2A.pdf", g, width = 7, height = 7, dpi = 400)
```

## Fig2B. Appropriate threshold on correlation prevents misclassification of missing types.

```{r}
null <- readr::read_rds('/Users/rf/Downloads/null.rds.gz')
reference <- readr::read_rds('/Users/rf/Downloads/reference.rds.gz')
query <- readr::read_rds('/Users/rf/Downloads/query.rds.gz')

ref <- t(reference %>% select(-label))
colnames(ref) <- 1:ncol(ref)
ref_c <- average_clusters(ref, reference$label, if_log = F)
...
# 
# cor_to_call(res4, metadata = pan_smartseq2_meta, 
#             cluster_col = "cell_type1")
# 
# res_allgenes <- cor_to_call(
#   correlation_matrix = res3, 
...
g<- cowplot::plot_grid(grid::grid.grabExpr(ComplexHeatmap::draw(a)),
                   grid::grid.grabExpr(ComplexHeatmap::draw(b)),
                   grid::grid.grabExpr(ComplexHeatmap::draw(c)), 
                   grid::grid.grabExpr(ComplexHeatmap::draw(d)),
                   ncol = 1,
                   labels  = c("all_genes", "seurat_var", "m3drop_var", "ref_var"))

ggsave("Fig2C.pdf", g, width = 7, height = 15, dpi = 400)
```
...
for (n in 1:length(tissue_both)) {
  ...
  if (sum(is.na(s_f@meta.data$free_annotation)) + sum(is.na(s_d@meta.data$free_annotation)) < (nrow(s_f@meta.data) + nrow(s_d@meta.data)) * 0.25) {
    ...
    avg <- ref_mca
    res <- clustify(s_d, avg, seurat_out = F, cluster_col = "free_annotation", dr = "tsne", query_genes = q)
  } else {
    print("use cell_ontology_class")
    if (length(unique(s_f@meta.data$cell_ontology_class)) < length(unique(s_d@meta.data$cell_ontology_class))) {
      s_temp <- s_d
    ...
    avg <- ref_mca
    res <- clustify(s_d, avg, seurat_out = F, cluster_col = "cell_ontology_class", dr = "tsne", query_genes = q)
  }  #avg <- cbind(avg, avg2)
  res2 <- cor_to_call(res)
  end <- proc.time()
  times[n] = end - start
...
```{r}
# saveRDS(f_all, "f_all_q.rds")
f_all <- readRDS("f_all_q.rds") %>% mutate(method = str_remove(method, "clustifyr_mca_")) %>% mutate(method = ifelse(method == "1", "orig", method))
a <- ggplot(f_all, aes(x = method, y = per, fill = method)) + 
  geom_boxplot(outlier.shape = NA) +
  ...
  ylab("accuracy") +
  ylim(0,1) + 
  scale_x_discrete(limits=c("orig", "15", "8", "4")) +
  xlab("# of cells") + 
  theme(legend.position="none")
  

b <- ggplot(f_all, aes(x = method, y = as.numeric(time), fill = method)) + 
  geom_boxplot(outlier.shape = NA) +
  #geom_jitter(width = 0.25) +
  cowplot::theme_cowplot() + 
  theme(axis.title.y.right = element_text(color = "red"), axis.text.x = element_text(size = 12, angle = 90, hjust = 1, vjust=0.5)) +
...
g <- cowplot::plot_grid(a,b)

g
#ggsave("Fig2D.pdf", g, width = 7, height = 7, dpi = 400)
```
...
#           feature = "cell_type1", x = "tSNE_1", y = "tSNE_2", do_label = F, do_legend = F)
...
res <- clustify_lists(
  ...
  marker = panm,
  cluster_info = clustifyrdata::pan_indrop_meta,
  cluster_col = "cell_type1",
  metric = "pct"
)
...
g <- cowplot::plot_grid(a0[[1]],a[[1]],b[[1]],c[[1]], ncol = 2, labels = c("spearman_cor","pct_express","ranked_cor_markers", "jaccard"))

g
ggsave("Fig3B.pdf", g, width = 10, height = 10, dpi = 400)
...
c <- plot_tsne(m2,
               x = "tSNE_1", y = "tSNE_2",
              feature= "type",
              alpha_col = "score", do_legend = F
              )
ggsave("Fig3C.pdf", c, width = 5, height = 5, dpi = 400)
```

## Fig3C. Consensus calls from scRNAseq and gene list on pancreatic data.