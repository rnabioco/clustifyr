...
test_that("plots can be generated", {
  plts <- plot_best_call(res, pbmc_meta, cluster_col = "classified")
...
test_that("plot_gene automatically plots all cells", {
  ...
  plts <- plot_gene(pbmc_matrix_small, pbmc_meta, genes = genes)
...
test_that("plot_gene checks for presence of gene name", {
  ...
  expect_error(expect_warning(plot_gene(
    pbmc_matrix_small,
    pbmc_meta %>% tibble::rownames_to_column("rn"),
    c("ZFP36L3"),
    cell_col = "rn",
...
test_that("plot_call works on defaults", {
  g <- plot_call(res, pbmc_meta, cluster_col = "classified")
